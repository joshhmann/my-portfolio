export const projects = [
    {
        id: 1,
        title: "CS-Butler - JavaScript Slackbot for Streamlined Support",
        description: "Developed a Slack app for automated support, improving team coordination and workflow efficiency using JavaScript, Slack API, JIRA API, and Opsgenie API.",
        techStack: ["JavaScript", "Slack API", "JIRA API", "Opsgenie API"],
        imageUrl: "/images/csbutler.jpg", // Replace with your actual image path
        liveDemoUrl: "#", // Replace with your actual URL, if available
        githubUrl: "#" // Replace with your actual URL, if available
    },
    {
        id: 2,
        title: "RMA Ticketing System",
        description: "Label maker for RMA parts",
        techStack: ["HTML", "CSS", "Javascript"],
        imageUrl: "/images/project4.png",
        liveDemoUrl: "https://sage-caramel-60a40c.netlify.app",
        githubUrl: "https://github.com/joshhmann/rma-ticket",
    },
    {
        id: 3,
        title: "Cloud-Native Blogging Platform (In Progress)",
        description: "Building a blogging platform with Docker and Kubernetes on AWS; focusing on CI/CD (GitHub Actions), monitoring (Prometheus/Grafana), and IaC (Terraform).",
        techStack: ["Docker", "Kubernetes", "AWS", "GitHub Actions", "Prometheus/Grafana", "Terraform"],
        imageUrl: "/images/cloud-native-blogging.jpg", // Replace with your actual image path
        liveDemoUrl: "#", // Replace with your actual URL, if available
        githubUrl: "#" // Replace with your actual URL, if available
    },
    {
        id: 4,
        title: "Game Server Cluster Deployment (In Progress)",
        description: "Automating Minecraft server deployment with Ansible and Terraform for Kubernetes. Implementing Prometheus and Grafana for real-time performance monitoring.",
        techStack: ["Ansible", "Terraform", "Kubernetes", "Prometheus/Grafana"],
        imageUrl: "/images/pterodactyl.png", // Replace with your actual image path
        liveDemoUrl: "#", // Replace with your actual URL, if available
        githubUrl: "#" // Replace with your actual URL, if available
    },
    {
        id: 4,
        title: "SignalApp - Video and Network Performance Analyzer",
        description: "Easily analyze and optimize your video and network performance with one click",
        techStack: ["Python", "Matplotlib", "Pandas", "Numpy"],
        imageUrl: "/images/signalapp.png", // Replace with your actual image path
        liveDemoUrl: "#", // Replace with your actual URL, if available
        githubUrl: "#" // Replace with your actual URL, if available
    },
    {
        id: 5,
        title: "Gemini Bot for Discord - Leveraging Google's Gemini API for Conversational AI in Discord Channels",
        description: "An interactive Discord bot using JavaScript and Node.js, integrating Google's Gemini API to enable rich, conversational experiences within Discord channels",
        techStack: ["Javascript", "discord.js", "Node.js", "Google Gemini API"],
        imageUrl: "/images/bingbot.png", // Replace with your actual image path
        liveDemoUrl: "#", // Replace with your actual URL, if available
        githubUrl: "#" // Replace with your actual URL, if available
    },
    // Add more projects here
];
